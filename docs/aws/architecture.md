...
| Amazon Athena | SQL query engine over the Gold Layer | Serverless, pay-per-query — no cluster to run for what is a small, infrequently-queried Gold Layer |

**Analytics is a separate concern from ingestion.** Glue/Athena
(`glue.tf`, `athena.tf`) only ever *read* the Gold Layer Parquet files
the pipeline above already produces — nothing in this layer writes
back into `data/`, and none of it participates in the Step Functions
dependency graph below. No dashboard/BI tool is deployed by this
Terraform — see `docs/aws/analytics.md` for that stack specifically
and why.
