{
  "Comment": "Manual, unconditional full rebuild of the Gold Layer (see gold/<source>/*.py) — one combined Parquet file per source (Eurostat, EEA, TED), each always built from EVERY country/year/pollutant currently available in that source's precursor stage (normalization for Eurostat, which has no transformation stage; transformation for EEA/TED). This supplements, not replaces, the automatic Gold rebuild historical.asl.json.tpl/update.asl.json.tpl already do at the end of each of their own source branches (conditional on that run having actually changed something, via main.py check-manifest-has-output) — this state machine always rebuilds all three regardless of whether anything changed recently, useful e.g. right after fixing a Gold-layer bug, without re-running historical/update just to get there. Manual only, never on a schedule. External input: {} — no fields are required or read; each branch always passes --discover, so there is nothing for a caller to select. The three sources are independent (no shared precursor, no ordering between them), so they run in parallel and one source failing doesn't stop the others.",
